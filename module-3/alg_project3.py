...
def main():
    ...
    """
    import time
    cancer_data = 'unifiedCancerData_111.csv'
    #  cancer_data = 'unifiedCancerData_290.csv'
    #  cancer_data = 'unifiedCancerData_896.csv'
    #  cancer_data = 'unifiedCancerData_3108.csv'

    with open(cancer_data, 'r') as f_data:
        cancer_list = [ [ ll.strip() for ll in l.split(',')] for l in f_data.readlines() ]
        cluster_list = [ alg_cluster.Cluster(set([d[0]]), float(d[1]), float(d[2]), float(d[3]), float(d[4])) for d in cancer_list ]
    ...
    time1 = time.time()
    print slow_closest_pairs(cluster_list)
    print fast_closest_pair(cluster_list)
    print hierarchical_clustering(cluster_list, 10)
    print kmeans_clustering(cluster_list, 40, 5)
